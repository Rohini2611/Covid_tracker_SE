...
const mapboxToken = 'pk.eyJ1IjoicnVjaGkxMCIsImEiOiJja3B6Ym5sbnkwZnFrMnZycjlleG9mMjRlIn0.5J2pDLXl73-Ny_8Bi61oLw';
...
map.on('load', () => {
    ...
    map.addLayer({
        ...
        source: {
            type: 'vector',
            url: 'mapbox://ruchi10.dl3lmj5j', // <--- Added the Map ID which was copied
        },
...
map.on('click', 'countries', (mapElement) => {
    const countryCode = mapElement.features[0].properties.ADM0_A3_IS; // Grabding the country code from the map properties.
...
const showNotification = (result, totalCases) => {
    const notification = new Notification("Message from CovidCare!", {
        body: `Cases have arised above ${result[1]} in the country ${result[0]}!!!
...
if (userData !== null) {
    window.addEventListener("load", () => {
        ...
        fetch(`https://disease.sh/v3/covid-19/countries/${userDataObj[0]}`)
            .then(res => {
                return res.json();
            })
            .then(data => {
                const totalCases = data.cases;
                if (data.cases > userDataObj[1]) {
